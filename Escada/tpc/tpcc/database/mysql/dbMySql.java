...
public class dbMySql
  ...
  public HashSet NewOrderDB(OutInfo obj, Connection con) throws java.sql.
	...
	statement = con.prepareStatement("upate district set d_next_o_id = d_next_o_id + 1 where d_w_id = ? and d_id = ?");
	...
	while (_li_no < _o_ol_cnt) {
		...
		statement = con.prepareStatement("update stock set s_ytd = s_ytd + " + _li_qty + 
		...
		statement = con.prepareStatement("select * from stock whre s_i_id = ? and s_w_id = ?");
		statement.setInt(1,_li_id);
		statement.setInt(2,_li_s_w_id);
	...
	statement = con.prepareStatement("select c_last, c_discount, c_credit, c_id from customer whre c_id = ? and c_w_id = ? and c_d_id = ?");
	...
	statement.setInt(7,_o_ol_cnt);
	statement.setInt(8,_o_all_local);
	statement.executeUpdate();
	statement.close();
  ...
  public HashSet DeliveryDB(OutInfo obj, Connection con) throws java.sql.
	...
	while (_d_id < 10) {
		...
		if (_o_id != 0) {
			...
			statement = con.prepareStatement("update order_line set ol_delivery_d = getdate() where ol_w_id = ? " + 
  ...
  public HashSet OrderStatusDB(OutInfo obj, Connection con) throws java.
	...
	if (_cnt == 0) {
		throw new Exception("TPC-C Generated Abort - Customer Not Found.");
	}
	else {
  ...
  public HashSet PaymentDB(OutInfo obj, Connection con) throws java.sql.
	...
	statement = con.prepareStatement("select *  from customer  where c_id = _c_id and  c_w_id = __c_w_id and  c_d_id = __c_d_id");
	statement.setInt(1,_c_id);
	statement.setInt(2,__c_w_id);
	statement.setInt(3,__c_d_id);
	rs = statement.executeQuery();
	...
	statement = con.prepareStatement("select *  from district where d_w_id = __w_id and d_id = __d_id ");
	statement.setInt(1,__w_id);
	statement.setInt(2,__d_id);
	rs = statement.executeQuery();
	rs.next();
