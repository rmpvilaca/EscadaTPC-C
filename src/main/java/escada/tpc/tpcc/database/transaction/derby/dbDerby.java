...
package escada.tpc.tpcc.database.transaction.derby;
...
/**
 * It is an interface to Derby, which based is based on the the
 * distributions of the TPC-C.
 */
public class dbDerby extends dbTPCCDatabase {

	private Logger logger = Logger.getLogger(dbDerby.class);
	...
	protected void InitTransaction(Connection con, String strTrans,
			String strAccess) throws SQLException {
	...
	protected void RollbackTransaction(Connection con, Exception dump,
			String strTrans, String strAccess) throws SQLException {
		Statement statement = null;
		try {
			Date NetStartTime = new Date();
