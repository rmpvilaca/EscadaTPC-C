...
public class CommonDatabase {
  ...
  /**
   * It defines the bahaivor of the database. 
   * It it is defined as virtual, in fact nothing is done to acquire and release connections.
   *
   * @param boolean (true) the database is virtual, (false) it is a normal database
   **/
  ...
  public static void setMaxConnection(int mConn)  {
	if (!virtualdatabase) ConnectionManager.setMaxConnection(mConn);
  }

  /**
   * It is used to enable or disable the connection pool.
   *
   * @param boolean (true) enables the connection pool or (false) disables the connection pool
   **/
  public static void setConnectionPool(boolean pool)
  {
      if (!virtualdatabase) connectionpool = pool;
  }
  ...
  {
      if (!virtualdatabase) ConnectionManager.setDriverName(dName);
  }
  ...
  {
      if (!virtualdatabase) ConnectionManager.setjdbcPath(jdbc);
  }
  ...
  {
      if (!virtualdatabase) ConnectionManager.setUserInfo(usr,pass);
  }
  ...
  **/
  public Connection getConnection() {
    if (!virtualdatabase) 
    	if (!connectionpool) return cn.createConnection();
	else {return cn.getConnection();}
    return (null);
  }
  ...
  **/
  public void returnConnection(Connection con) {
     if (!virtualdatabase) 
	if (!connectionpool) cn.closeConnection(con);
	else cn.returnConnection(con);
  }
}
