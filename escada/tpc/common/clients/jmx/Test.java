...
public class Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Set<ObjectName> beans = mbsc.queryNames(new ObjectName("escada.tpc.common.clients.jmx:type=ClientEmulationStartup"), null);
			
			System.out.println("beans: "+beans.toString()+"\n");
			
			for ( ObjectName b: beans ) {
				ClientEmulationStartupMBean ces = (ClientEmulationStartupMBean) MBeanServerInvocationHandler.newProxyInstance(mbsc, b, ClientEmulationStartupMBean.class, false);
				if ( 1 == args.length ) {
